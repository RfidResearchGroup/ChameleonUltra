...
class ChameleonCMD:
    ...
    def hf14a_sniff(self, timeout_ms: int = 5000):
        """
        Capture ISO14443A reader frames while CU is in tag emulation mode.

        Place the CU near a reader — all commands the reader sends are
        captured and returned as a list of raw frame byte arrays.

        :param timeout_ms: Listen duration in ms (1-30000, default 5000)
        :return: Response object. resp.data contains packed frames:
                 each frame: [2 bytes bit-count BE][N bytes data]
        """
        timeout_ms = max(1, min(30000, timeout_ms))
        payload = bytes([(timeout_ms >> 8) & 0xFF, timeout_ms & 0xFF])
        # Pass timeout_s = sniff duration + 2s safety margin so send_cmd_sync
        # doesn't give up before the firmware finishes
