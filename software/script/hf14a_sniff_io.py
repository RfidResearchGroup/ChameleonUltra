"""
hf14a_sniff_io.py — sniff capture I/O for chameleon_cli_unit.py

Three additions to integrate into the existing CLI:

  1. Patch HF14ASniff.args_parser() to add --save / --no-decode
  2. Patch HF14ASniff.on_exec()  to write native file when --save given
  3. New class HF14ASniffLoad     (@hf_14a.command('sniff-load'))
  4. New class HF14ASniffExportPm3 (@hf_14a.command('sniff-export-pm3'))
...
Native file format ("CUSN" — Chameleon Ultra Sniff Native, v1):
    ...
    16      N     buffer         raw resp.data from hf14a_sniff() (packed format
...
def parse_cusn_buffer(buffer: bytes, has_directions: bool = True):
    ...
    Same logic as HF14ASniff.on_exec but factored out for reuse by load/export.
