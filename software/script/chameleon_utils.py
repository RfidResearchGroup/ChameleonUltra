    ...
    def print_help(self):
        ...
        if len(options) > 0 and options[0].strip() == "positional arguments:":
            ...
            positional_args[0] = positional_args[0].replace(
                "positional arguments:", color_string((CG, "positional arguments:"))
            )
        ...
        if len(options) > 0:
            ...
            options[0] = options[0].replace(
                "optional arguments:", color_string((CG, "optional arguments:"))
            )
            if len(options) > 1:
                options.append("")
            print("\n".join(options))
...
def execute_tool(tool_name, args):
    ...
    tool_path = SCRIPT_BIN_DIR / tool_executable
    if not tool_path.exists():
        fallback_tool_path = CWD_BIN_DIR / tool_executable
        if fallback_tool_path.exists():
            tool_path = fallback_tool_path

    cmd_recover_list = [str(tool_path)]
