...
static void update_config_crc(void) { calc_14a_crc_lut((uint8_t *)&config, sizeof(config), (uint8_t *)&m_config_crc); }
...
void settings_update_version_for_config(void) { config.version = SETTINGS_CURRENT_VERSION; }
...
// add on version5
void settings_init_ble_pairing_enable_config(void) { config.ble_pairing_enable = false; }
...
void settings_migrate(void)
{
    switch (config.version) {
        ...
        case 4:
             ...
             * `settings_update_version_for_config()` and `break` statements should only be used on the last migration
             * step, all the previous steps must fall through to the next case.
             */

            settings_update_version_for_config();
            break;
        default:
            NRF_LOG_ERROR("Unsupported configuration migration attempted! (%d -> %d)", config.version,
...
void settings_load_config(void)
{
    ...
    if (ret) {
        ...
        // After the reading is complete, we first save a copy of the current CRC, which can be used as a reference for
        // comparison of changes when saving later
        update_config_crc();
    }
    else {
    ...
    if (config.version > SETTINGS_CURRENT_VERSION) {
        NRF_LOG_WARNING(
            "Config version %d is greater than current firmware supports (%d). Default config will be loaded.",
            config.version, SETTINGS_CURRENT_VERSION);
        settings_init_config();
    }
    if (config.version < SETTINGS_CURRENT_VERSION) {
        NRF_LOG_INFO("Config version (%d) is not latest, performing migration to %d", config.version,
                     SETTINGS_CURRENT_VERSION);
        settings_migrate();
    }
...
{
    // We are saving the configuration, we need to calculate the crc code of the current configuration to judge whether
...
uint8_t settings_get_animation_config() { return config.animation_config; }

void settings_set_animation_config(uint8_t value) { config.animation_config = value; }
...
uint8_t *settings_get_ble_connect_key(void) { return config.ble_connect_key; }
 ...
 */
void settings_set_ble_connect_key(uint8_t *key) { memcpy(config.ble_connect_key, key, BLE_PAIRING_KEY_LEN); }

void settings_set_ble_pairing_enable(bool enable) { config.ble_pairing_enable = enable; }

bool settings_get_ble_pairing_enable(void) { return config.ble_pairing_enable; }
