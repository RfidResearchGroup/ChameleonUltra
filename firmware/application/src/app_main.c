...
static void button_pin_handler(nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
    ...
    if (mode == DEVICE_MODE_TAG) {
        static nrf_drv_gpiote_pin_t
            pin_static;    // Use static internal variables to store the GPIO where the current event occurred
...
static void system_off_enter(void)
{
    ...
    // Check whether there are low -frequency fields, solving very strong field signals during dormancy have always
    // caused the comparator to be at a high level input state, so that the problem of uprising the rising edge cannot
    // be awakened.
    if (is_lf_field_exists()) {
        // Close the comparator
        nrf_drv_lpcomp_disable();
        // Set the reason for Reset. After restarting, you need to get this reason to avoid misjudgment from the source
        // of wake up.
    ...
    // When starting debugging, we should disable low power consumption state values, or simply not enter low power
    // consumption
    ret = sd_power_system_off();

    // OK, here is very important. If you open the log output and enable RTT, you will not check the error of the low
    // power mode
...
static void check_wakeup_src(void)
{
    ...
    /*
     * Note: The hibernation described below is deep hibernation, stopping any non-wakeup source peripherals and
     * stopping the CPU to achieve the lowest power consumption
     *
     * If the wake-up source is a button, then you need to turn on BLE broadcast until the button stops clicking after a
     * period of time to hibernate If the wake-up source is the field of the analog card, it is not necessary to turn on
     * BLE broadcast until the analog card ends and then hibernate. If the wake-up source is USB, then keep BLE on and
     * no hibernation until USB is unplugged If the wakeup source is the first time to access the battery, then do
     * nothing and go directly to hibernation
     *
     * Hint: The above; logic is the logic processed in the wake-up phase, the rest of the logic is converted to the
    ...
    if (m_reset_source & NRF_POWER_RESETREAS_OFF_MASK) {
        ...
        // It is currently the wake-up system of the emulation card event, we can make the strong lights on the field
        // first
        TAG_FIELD_LED_ON();
...
static void offline_status_ok(void) { offline_status_blink_color(RGB_GREEN); }
...
int main(void)
{
    ...
    check_wakeup_src();  // Detect wake-up source and decide BLE broadcast and subsequent hibernation action according
                         // to the wake-up source
