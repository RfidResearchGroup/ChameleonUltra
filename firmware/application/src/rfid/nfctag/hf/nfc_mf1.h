...
typedef struct {
    /**
     * Normal write mode (write normally according to the current state, affected by the control bit and the back door
     * card) Deny write mode (similar to control bit lock, directly reject any write, return nack) Fraudulent writing
     * mode (on the surface, returning ack indicates that the writing is successful, but in fact, even RAM is not
     * written) Shadow write mode (write to RAM, and return ack to indicate success, but not save to flash)
     *  @see nfc_tag_mf1_write_mode_t
     */
    nfc_tag_mf1_write_mode_t mode_block_write;
    /**
     * In communication mode, if the interoperability mode is enabled, some information in the M1 sector data will be
     * used Otherwise, the information defined by the anti -collision phase will be used alone. This setting is limited
     * to 4 bytes NFC_TAG_14A_UID_SINGLE_SIZE. Unless there are any documents that indicate 0 blocks of 7 -bytes and 10
     * -byte cards with relevant SAK specifications
     */
    uint8_t use_mf1_coll_res : 1;
    /**
     * Chinese Gen1A back door card mode, the highest permissions of this mode
     * After turning on, the response of the back door card operation instruction, and all operations are released
     * directly, not affected by the Mode_block_write and control bit
     */
 ...
 * If the byte alignment is not performed, the abnormalities of visiting the cross -border will occur when the structure
 * is directly stated and saved directly to the Flash
 */
...
typedef struct {
    ...
    // The length of the data after packing, according to the above news, it can be seen that the maximum number of BITs
    // of MF1 communication does Therefore, a byte is sufficient to store the length value
