...
#define NRFX_NFCT_RX_BYTES                                                                                        \
    do {                                                                                                          \
        ...
        NRF_NFCT->INTENSET                                                                                        \
            = (NRF_NFCT_INT_RXFRAMESTART_MASK | NRF_NFCT_INT_RXFRAMEEND_MASK | NRF_NFCT_INT_RXERROR_MASK);        \
    ...
    while (1) {
        ...
        for (uiBitPos = 0; uiBitPos < 8; uiBitPos++) {
            ...
            *pbtFrame = byte_mirror[btFrame];
            ...
            uiDataPos++;
            // Test if we are done
            if (szBitsLeft < 9) return szFrameBits;
            szBitsLeft -= 8;
        }
 ...
 * @retval :The data length of the bitstream packaging, note that the length of the data area is the length of the data
 * area.retval / 8
 */
uint8_t nfc_tag_14a_unwrap_frame(const uint8_t *pbtFrame, const size_t szFrameBits, uint8_t *pbtRx, uint8_t *pbtRxPar)
{
    ...
    while (1) {
        for (uiBitPos = 0; uiBitPos < 8; uiBitPos++) {
            ...
            uiDataPos++;
            // Test if we are done
            if (szBitsLeft < 9) return szRxBits;
            szBitsLeft -= 9;
        }
        // Every 8 data bytes we lose one frame byte to the parities
        pbtFramePos++;
...
void nfc_tag_14a_data_process(uint8_t *p_data)
{
    ...
    nfc_tag_14a_coll_res_reference_t *auto_coll_res
        = m_tag_handler.get_coll_res != NULL ? m_tag_handler.get_coll_res() : NULL;

    // I don't know why, here the CPU must run empty for a period of time before the data can be received normally.
    // If you have any problems with the receiving data, please try to restore this. This is a problem found in 2021,
    // but it disappeared again in 2022 It may be due to the update of the SDK version for (int i = 0; i < 88; i++)
    // __NOP();
    ...
    if (szDataBits <= 8) {
        ...
        // Temporary through: Wupa response in non -choice state, no matter what state is in the state, you can use the
        // Wupa instruction to wake up
        if ((szDataBits == 7) && ((isREQA && m_tag_state_14a != NFC_TAG_STATE_14A_HALTED) || isWUPA)) {
            ...
            if (auto_coll_res != NULL) {
                // The status machine is set to the preparation state, and the next operation is to enter the card
                // selection link
            ...
            // TODOHere you can match some other instructions, call back some registered processing functions to handle
            // this logic separately Normal communication process will not have N bits of frames, because it is the anti
            // -conflict frame used in the 14A protocol for BIT So you can handle this protocol frame separately here to
            // realize the tag similar to the UID back door card (Chinese Magic) Note that if we find REQA or WUPA, we
            // will not repeat the processing (only the special ratio special frame)
            if ((!isREQA && !isWUPA) && m_tag_handler.cb_state != NULL) {
                // If the 7bit processor is registered and successfully processed this command, the state machine update
                // is completed
    ...
    switch (m_tag_state_14a) {
        // If you do not handle any tasks in the idle state and the dormant state, let the news from the stars go with
        // the wind ~
        ...
        case NFC_TAG_STATE_14A_READY: {
            ...
            if (szDataBits >= 16) {
                ...
                switch (*auto_coll_res->size) {
                    ...
                    case NFC_TAG_14A_UID_TRIPLE_SIZE: {
                        ...
                        else if (level == NFC_TAG_14A_CASCADE_LEVEL_2) {  // The second level, there is still the
                                                                          // remaining first -level joint
                            uid[0] = NFC_TAG_14A_CASCADE_CT;
            ...
            if (szDataBits == 72 && p_data[1] == 0x70) {
                if (memcmp(&p_data[2], uid, 4) == 0) {
                    bool cl_finished
                        = (*auto_coll_res->size == NFC_TAG_14A_UID_SINGLE_SIZE && level == NFC_TAG_14A_CASCADE_LEVEL_1)
                          || (*auto_coll_res->size == NFC_TAG_14A_UID_DOUBLE_SIZE
                              && level == NFC_TAG_14A_CASCADE_LEVEL_2)
                          || (*auto_coll_res->size == NFC_TAG_14A_UID_TRIPLE_SIZE
                              && level == NFC_TAG_14A_CASCADE_LEVEL_3);
                    ...
                    if (cl_finished) {
                        ...
                        // It is necessary to continue the level, so we need to respond to a data that marks the
                        // incomplete UID in SAK
        ...
        case NFC_TAG_STATE_14A_ACTIVE: {
            // You need to judge whether you have received instructions that need to be handled directly without
            // forwarding
            if (szDataBits == 32) {
                // Halt instruction
                ...
                if (p_data[0] == NFC_TAG_14A_CMD_RATS && nfc_tag_14a_checks_crc(p_data, 4)) {
                    ...
                    if (auto_coll_res->ats->length > 0) {
                        ...
                        // If the FSD is larger than the set of ATS, then returns normal ATS data, otherwise the data of
                        // the FSD limited length will be returned
                        uint8_t len = fsd >= auto_coll_res->ats->length ? auto_coll_res->ats->length : fsd;
                        // Back to ATS data according to FSD, FSD is the largest frame size supported by PCD. After
                        // removing CRC, it is the actual data frame size support
                        nfc_tag_14a_tx_bytes(auto_coll_res->ats->data, len, true);
                    }
                    else {
                        nfc_tag_14a_tx_nbit(NAK_INVALID_OPERATION_TBIV, 4);
                    }
                    // After handling the explicitly sending RATS instructions outside the outside, wait directly for
                    // the next round of communication
                    return;
                }
            }
            // No processing is successful, it may be some other data. You need to re-post processing
            if (m_tag_handler.cb_state
                != NULL) {  // Activation status, transfer the message to other registered processor processing
                m_tag_handler.cb_state(p_data, szDataBits);
                break;
            }
        }
...
void nfc_tag_14a_event_callback(nrfx_nfct_evt_t const *p_event)
{
    ...
    switch (p_event->evt_id) {
        case NRFX_NFCT_EVT_FIELD_DETECTED: {
            ...
            // Turn off the automatic anti -collision, MCU management all the interaction process, and then enable the
            // NFC peripherals so that Io can be performed after enable
        ...
        case NRFX_NFCT_EVT_RX_FRAMEEND: {
            ...
            // TODO Remember a bug, if you do not reply to the message after receiving the message, you need to manually
            // enable you
            //   Otherwise, the nrfx_nfct_evt_tx_frameend conditions above will not be triggered, and nrfx_nfct_rx_bytes
            //   will not be called
            ...
            // This function processes the data sent by the card reader, and then read that you don't need to reply to
            // the card reader. If you need it, reply Don't reply if you don't need it, it makes sense, right?This is
            // science.
            nfc_tag_14a_data_process(m_nfc_rx_buffer);
            // The above prompt tells us that when we do not need to reply to the card reader, we need to manually
            // enable it
            if (!m_is_responded) {
                nfc_fdt_reset();
                NRFX_NFCT_RX_BYTES
        ...
        case NRFX_NFCT_EVT_ERROR: {
            // According to the error reasons, the log prints to help the development of the possibilities during
            // development
            switch (p_event->params.error.reason) {
                case NRFX_NFCT_ERROR_FRAMEDELAYTIMEOUT: {
                    // If we respond to the label in the communication window, but we did not respond in time, then we
                    // need to make an error printing
                    //  If this error appears very frequently, it may be that the MCU processing speed does not keep up.
                    //  At this time, the developer needs to optimize the code
                    if (m_is_responded) {
 ...
 */
void nfc_tag_14a_set_state(nfc_tag_14a_state_t state) { m_tag_state_14a = state; }
...
{
    return uid_length == NFC_TAG_14A_UID_SINGLE_SIZE || uid_length == NFC_TAG_14A_UID_DOUBLE_SIZE
