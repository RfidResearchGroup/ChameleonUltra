...
#define NAK_OTHER_ERROR \
    0x06  // This is not defined in the manual, it belongs to the color -changing dragon special (may need the sector)
...
typedef enum {
    ...
    NFC_TAG_STATE_14A_ACTIVE,  // Select cards or other instructions to enter the working status, which can receive all
                               // data
    NFC_TAG_STATE_14A_HALTED,  // The label stops working status and can only be awakened by Halt or other special
                               // instructions (non -labels)
} nfc_tag_14a_state_t;
