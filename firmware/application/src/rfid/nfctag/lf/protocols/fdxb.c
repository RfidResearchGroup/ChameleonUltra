...
uint8_t fdxb_t55xx_writer(uint8_t *fdxb_data, uint32_t *blks) {
    ...
    // Blocks 1-4: Pack 128-bit frame into four 32-bit words (frame bits 0-31 first)
    blks[1] = (uint32_t)(raw_lo & 0xFFFFFFFF);          // bits 0-31
    blks[2] = (uint32_t)((raw_lo >> 32) & 0xFFFFFFFF);  // bits 32-63
    blks[3] = (uint32_t)(raw_hi & 0xFFFFFFFF);          // bits 64-95
    blks[4] = (uint32_t)((raw_hi >> 32) & 0xFFFFFFFF);  // bits 96-127
