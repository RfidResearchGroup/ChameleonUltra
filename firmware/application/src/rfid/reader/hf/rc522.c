 ...
 * @retval none
 */
void pcd_14a_reader_timeout_set(uint16_t timeout_ms) { g_com_timeout_ms = timeout_ms; }
 ...
 * @retval Timeout
 */
uint16_t pcd_14a_reader_timeout_get() { return g_com_timeout_ms; }
 ...
 */
uint8_t pcd_14a_reader_bytes_transfer(uint8_t Command, uint8_t *pIn, uint8_t InLenByte, uint8_t *pOut,
                                      uint16_t *pOutLenBit, uint16_t maxOutLenBit)
{
    ...
    write_register_single(CommandReg, PCD_IDLE);  //  Flushbuffer clearing the internal FIFO read and writing pointer
                                                  //  and ErRreg's Bufferovfl logo position is cleared
    clear_register_mask(ComIrqReg,
                        0x80);  //  When Set1 is cleared, the shielding position of commonricqreg is clear zero
    ...
    if (Command == PCD_TRANSCEIVE) {
        set_register_mask(BitFramingReg, 0x80);  // StartSend places to start the data to send this bit and send and
                                                 // receive commands when it is valid
    }
    ...
    *pOutLenBit = 0;
    ...
    do {
        n = read_register_single(ComIrqReg);  // Read the communication interrupt register to determine whether the
                                              // current IO task is completed!
        not_timeout = NO_TIMEOUT_1MS(g_timeout_auto_timer, g_com_timeout_ms);
    } while (not_timeout
             && (!(n & waitFor)));  // Exit conditions: timeout interruption, interrupt with empty command commands
    // NRF_LOG_INFO("N = %02x\n", n);
    ...
    if (not_timeout) {
        ...
        if (n & 0x02) {
            ...
            if (pcd_err_val & 0x01) {          // ProtocolErr Error only appears in the following two cases:
                if (Command == PCD_AUTHENT) {  // During the execution of the MFAUTHENT command, if the number of bytes
                                               // received by a data stream, the position of the place
                    // Therefore, we need to deal with it well, assuming that there are problems during the verification
                    // process, then we need to think that this is normal
                    status = STATUS_MF_ERR_AUTH;
                }
                else {  // If the SOF is wrong, the position is set up and the receiver is automatically cleared during
                        // the start -up stage, which is effective at the rate of 106kbd
            ...
            if (Command == PCD_TRANSCEIVE) {
                ...
                }  // N -byte number minus 1 (last byte)+ the number of bits of the last bit The total number of data
                   // readings read
                else {
                    *pOutLenBit = n * 8;
                }  // Finally received the entire bytes received by the byte valid
...
*/
uint8_t pcd_14a_reader_bits_transfer(uint8_t *pTx, uint16_t szTxBits, uint8_t *pTxPar, uint8_t *pRx, uint8_t *pRxPar,
                                     uint16_t *pRxLenBit, uint16_t szRxLenBitMax)
{
    static uint8_t buffer[DEF_FIFO_LENGTH];
    uint8_t status = 0, modulus = 0, i = 0, dataLen = 0;
        ...
        *pRxLenBit = *pRxLenBit - modulus;
        ...
        if (*pRxLenBit > szRxLenBitMax) {
            NRF_LOG_INFO("pcd_14a_reader_bits_transfer decode parity data overflow: %d, max = %d\n", *pRxLenBit,
                         szRxLenBitMax);
            // There must be an overflow here, and the length of the data that is valid is reset to avoid misjudgment
            // from external calls.
            *pRxLenBit = 0;
 ...
 *          POUTLENBIT: Bit the length of the data
 *          FLAGS:
 *            - PCD_TRANSMIT_FLAG_NO_RESET_MF_CRYPTO1_ON: Do not reset MFCrypto1On
 * @retval : Status value mi_ok, successful
 */
uint8_t pcd_14a_reader_bytes_transfer_flags(uint8_t Command, uint8_t *pIn, uint8_t InLenByte, uint8_t *pOut,
                                            uint16_t *pOutLenBit, uint16_t maxOutLenBit, uint32_t flags)
{
    uint8_t status = STATUS_HF_ERR_STAT;
    uint8_t waitFor = 0x00;
    uint8_t lastBits = 0;
    ...
    *pOutLenBit = 0;
    ...
    if ((flags & PCD_TRANSMIT_FLAG_NO_RESET_MF_CRYPTO1_ON) == 0 && status != STATUS_HF_TAG_OK) {
        // If there are certain operations,
        // We may need to remove MFCrypto1On This register logo,
        // Because it may be because of the error encryption communication caused by verification
        clear_register_mask(Status2Reg, 0x08);
...
uint8_t pcd_14a_reader_fast_select(picc_14a_tag_t *tag)
{
    ...
    for (; cascade_level < tag->cascade; cascade_level++) {
        ...
        status = pcd_14a_reader_bytes_transfer(PCD_TRANSCEIVE, dat_buff, sizeof(dat_buff), dat_buff, &dat_len,
...
uint8_t pcd_14a_reader_scan_once(picc_14a_tag_t *tag)
{
    ...
    for (; do_cascade; cascade_level++) {
        ...
        status
            = pcd_14a_reader_bytes_transfer(PCD_TRANSCEIVE, sel_all, sizeof(sel_all), resp, &len, U8ARR_BIT_LEN(resp));
        ...
        if (status != STATUS_HF_TAG_OK) {
            ...
            // So do not solve the collision for the time being, but directly inform the user that the user guarantees
            // that there is only one card in the field
        ...
        status
            = pcd_14a_reader_bytes_transfer(PCD_TRANSCEIVE, sel_uid, sizeof(sel_uid), resp, &len, U8ARR_BIT_LEN(resp));
        if (status != STATUS_HF_TAG_OK) {
        ...
        // In the next cycle, we need to make an increased level, return to the end of the anti -rushing collision, and
        // complete the level
 ...
 *               Within the data block reading and writing operation range, if the field is turned off or the card is
 * halt or the collision is re -anti -collision, It will lose the back door authority, and you need to call this
 * function to restart.
 *
...
/**
* @brief  : Perform the UFUID card and block the back door instructions to make it a normal card without responding to
the back door instruction
...
uint8_t pcd_14a_reader_gen1a_uplock(void)
{
    ...
    uint8_t uplock_2[]
        = {0x85, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x18, 0x47};

    uint8_t recvbuf[1] = {0x00};

    status = pcd_14a_reader_bytes_transfer(PCD_TRANSCEIVE, uplock_1, sizeof(uplock_1), recvbuf, &rx_length,
                                           U8ARR_BIT_LEN(recvbuf));
    if (!(status == STATUS_HF_TAG_OK && rx_length == 4 && recvbuf[0] == 0x0A)) {
        NRF_LOG_INFO("UPLOCK1(UFUID) FAILED!\n");
        return STATUS_HF_ERR_STAT;
    }

    status = pcd_14a_reader_bytes_transfer(PCD_TRANSCEIVE, uplock_2, sizeof(uplock_2), recvbuf, &rx_length,
                                           U8ARR_BIT_LEN(recvbuf));
    if (!(status == STATUS_HF_TAG_OK && rx_length == 4 && recvbuf[0] == 0x0A)) {
 ...
 * @retval : The status value STATUS_HF_TAG_OK is successful, tag_errauth fails, and other returns indicate some
 * abnormalities related to communication errors!
 */
 ...
 */
void pcd_14a_reader_mf1_unauth(void) { clear_register_mask(Status2Reg, 0x08); }
...
uint16_t pcd_14a_reader_mf1_read_by_cmd(uint8_t cmd, uint8_t addr, uint8_t *p)
{
    ...
    if (status == STATUS_HF_TAG_OK) {
        if (len == 0x90 /* 0x90 = 144bits */) {
            ...
            // The data passed back is wrong, which may be an environmental factors or cards that do not comply with
            // specifications! Or the control bit affects reading!
            status = STATUS_HF_ERR_STAT;
        }
    }
    return status;
...
uint8_t pcd_14a_reader_mf1_write_by_cmd(uint8_t cmd, uint8_t addr, uint8_t *p)
{
    ...
    if (status == STATUS_HF_TAG_OK) {
        ...
        status
            = pcd_14a_reader_bytes_transfer(PCD_TRANSCEIVE, dat_buff, 18, dat_buff, &dat_len, U8ARR_BIT_LEN(dat_buff));
        // The communication fails, the reason is returned directly
        if (status != STATUS_HF_TAG_OK) {
            return status;
        }
        // The communication is successful, we need to determine whether the card is successfully processed after
        // receiving the data And reply ACK
        if ((dat_len != 4) || ((dat_buff[0] & 0x0F) != 0x0A)) {
...
uint8_t pcd_14a_reader_mf1_manipulate_value_block(uint8_t operator, uint8_t addr, int32_t operand)
{
    ...
    status = pcd_14a_reader_bytes_transfer_flags(PCD_TRANSCEIVE, dat_buff, 6, dat_buff, &dat_len,
                                                 U8ARR_BIT_LEN(dat_buff), PCD_TRANSMIT_FLAG_NO_RESET_MF_CRYPTO1_ON);
...
uint8_t pcd_14a_reader_mf1_transfer_value_block(uint8_t addr)
{
    uint8_t status;
    uint16_t dat_len;
    uint8_t dat_buff[4] = {PICC_TRANSFER, addr};
 ...
 */
inline void pcd_14a_reader_parity_on(void) { clear_register_mask(MfRxReg, 0x10); }

/**
 * @brief  : Disable the parity bit check.
 */
inline void pcd_14a_reader_parity_off(void) { set_register_mask(MfRxReg, 0x10); }
 ...
 *             Note: This address may refer to the stack, and it is not guaranteed that it must be a global and
 * effective address. This address is based on tag The location of the memory determines the life cycle.
 *
 */
 ...
 *           if MCU The performance is sufficient, it is recommended to put it MCU Calculated above to make the
 * calculation process smoother if MCU Insufficient performance, it is recommended to put it in 522 Calculated above,
 * alleviate the calculation pressure of MCU
 *
 */
inline void pcd_14a_reader_crc_computer(uint8_t use522CalcCRC) { m_crc_computer = use522CalcCRC; }

/**
 * @brief    : The hf 14a raw command implementation function can be used to send the 14A command with the specified
 ...
 * @param    :checkCrc           : Is CRC verified after receiving data? If CRC verification is enabled, CRC bytes will
 * be automatically removed after verification is completed.
 * @param    :waitRespTimeout    : If waitResp is enabled, this parameter will be the timeout value to wait for the tag
 * to respond
 ...
 *
 */
uint8_t pcd_14a_reader_raw_cmd(bool openRFField, bool waitResp, bool appendCrc, bool autoSelect, bool keepField,
                               bool checkCrc, uint16_t waitRespTimeout, uint16_t szDataSendBits, uint8_t *pDataSend,
                               uint8_t *pDataRecv, uint16_t *pszDataRecv, uint16_t szDataRecvBitMax)
{
    ...
    *pszDataRecv = 0;
    ...
    if (appendCrc) {
        ...
        if (szDataSendBits > ((DEF_FIFO_LENGTH - DEF_CRC_LENGTH) * 8)) {
            // Note: Adding CRC requires at least two bytes of free space. If the transmitted data is already greater
            // than or equal to 64, an error needs to be returned
    ...
    if (szDataSendBits) {
        // If there is no need to receive data, the data receiving cache needs to be empty, otherwise a specified
        // timeout value needs to be set Caching old timeout values
        ...
        if (szDataSendBits % 8) {
            status = pcd_14a_reader_bits_transfer(pDataSend, szDataSendBits, NULL, pDataRecv, NULL, pszDataRecv,
                                                  szDataRecvBitMax);
        }
        else {
            status = pcd_14a_reader_bytes_transfer(PCD_TRANSCEIVE, pDataSend, szDataSendBits / 8, pDataRecv,
                                                   pszDataRecv, szDataRecvBitMax);
        }

        // If we need to receive data, we need to perform further operations on the data based on the remaining
        // configuration after receiving it
        if (waitResp) {
                ...
                if (finalRecvBytes >= 3) {  // Ensure at least three bytes (one byte of data+two bytes of CRC)
                    ...
                    if (pDataRecv[finalRecvBytes - 2] != crc_buff[0] || pDataRecv[finalRecvBytes - 1] != crc_buff[1]) {
                        ...
                        *pszDataRecv = 0;
                        ...
                        // If the CRC needs to be verified by the device and the device determines that the CRC is
                        // normal, we will return the data without CRC
