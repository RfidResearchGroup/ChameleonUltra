...
/**
 * @brief    : Calculate the distance value of the random number, and calculate the multi -type table based on real
 * -time Official comment:x,y valid tag nonces, then prng_successor(x, nonce_distance(x, y)) = y
...
static void nonce_distance(uint32_t *msb, uint32_t *lsb)
{
    ...
    for (uint16_t i = 1; i; ++i) {
        ...
        pos = (x & 0xff) << 8 | x >> 8;
        // To judge the coordinates, we take out the corresponding value and set the logo bit with the value of the
        // value
...
static bool check_lfsr_prng(uint32_t nonce)
{
    ...
    // The coordinates are passed in direct operation, and the rumors are also indirectly spread by the passing
    // parameters.
 ...
 *
 */
static uint8_t send_cmd(struct Crypto1State *pcs, uint8_t encrypted, uint8_t cmd, uint8_t data, uint8_t *status,
                        uint8_t *answer, uint8_t *answer_parity, uint16_t answer_max_bit)
{
    ...
    if (pcs && encrypted) {
        ...
        *status = pcd_14a_reader_bits_transfer(ecmd, 32, par, answer, answer_parity, &len, answer_max_bit);
    }
    else {
    ...
    if (encrypted == CRYPT_ALL) {
        if (len == 8) {
            ...
            for (pos = 0; pos < len % 8; pos++) answer[pos] = crypto1_byte(pcs, 0x00, 0) ^ answer[pos];
        }
    }
 ...
 *
 */
int authex(struct Crypto1State *pcs, uint32_t uid, uint8_t blockNo, uint8_t keyType, uint64_t ui64Key, uint8_t isNested,
           uint32_t *ntptr)
{
 ...
 * @retval The final NT value determined, if the card clock cannot be synchronized, returns the corresponding abnormal
 * code, otherwise return hf_tag_ok
 * -------------------
 ...
 * 1. The electromagnetic environment where it is located is very complicated, resulting in the process of getting the
 * power charging of the card to complete the communication.
 * 2. The card is repaired for the loopholes for the heavy attack attack, and the card is no longer a replaceable attack
 * set to take the same response
 ...
 * There is basically no solution in this situation. It is recommended to close the non -critical interruption and task
 * scheduling outside this code
 */
static uint8_t darkside_select_nonces(picc_14a_tag_t *tag, uint8_t block, uint8_t keytype, uint32_t *nt,
...
#define NT_COUNT 15
    ...
    for (i = 0; i < NT_COUNT; i++) {
        ...
        //  1. The antenna is powered off for a long time to ensure that the card is completely powered off, otherwise
        //  the pseudo -random number generator of the card cannot be reset
        //  2. Moderate power -off time, don't be too long, it will affect efficiency, and don't be too short.
        reset_radio_field_with_delay();
        // After the power is completely disconnected, we will select the card quickly and compress the verification
 ...
 * @retval   : Collect successfully returning hf_tag_ok, verify that the corresponding abnormal code is not successfully
 * returned
 *
 */
uint8_t darkside_recover_key(uint8_t targetBlk, uint8_t targetTyp, uint8_t firstRecover, uint8_t ntSyncMax,
                             DarksideCore_t *dc, mf1_darkside_status_t *darkside_status)
{
    ...
    // Control variable
    uint8_t resync_count = 0x00;  // This variable is responsible for counting the current sync clock with the number of
                                  // attempts of synchronous NT
    uint8_t received_nack = 0x00;  // This variable is responsible for whether it receives the reply from Nack
    uint8_t par = 0x00;  // This variable is responsible for increasing the puppet school inspection, and the reply of
                         // the collision card
    uint8_t status = 0x00;  // This variable is responsible for saving card communication status
    uint16_t len = 0x00;    // This variable is responsible for saving the data of the card in the communication process
                            // to respond to the length of the card
 ...
 *               The shorter the delay, the more you get the process of processing the antenna restart, and then deal
 * with the follow -up business
 * @param    :delay_ms   : The specific value of the delay, the unit is milliseconds
 * @retval   : none
 *
 */
void antenna_switch_delay(uint32_t delay_ms) { g_ant_reset_delay = delay_ms; }
 ...
 * @retval   : If support, return nested_tag_is_static, if it is not supported, if support, return nested_tag_is_static,
 * if it is not supported, if it is not supported, it is not supported, If support, return nested_tag_is_static, if not
 * support,
 *
 */
uint8_t check_static_prng(bool *is_static)
{
    ...
    // If you do not re -set the field, some cards will always provide a static NT when maintaining the function in the
    // field Therefore, resetting here is very important.
    reset_radio_field_with_delay();
...
/**
 * @brief    : Determine whether this card supports the most common, weaker, and easiest nested attack, or static, or
 * hardnested
...
/**
 * @brief    :The distance should be measured before the Nested attack. If the distance is appropriate, you can quickly
 * crack
...
static uint8_t measure_distance(uint64_t u64Key, uint8_t block, uint8_t type, uint32_t *distance)
{
    ...
    do {
        ...
        // If you really encounter the same NT, then it can only be explained that this card is a ST card for special
        // firmware
        if (nt1 == nt2) {
...
/**
 * @brief    : Nested core, used to collect random numbers. This function is only responsible for collecting, not
 * responsible for conversion and analysis as KS
 ...
 *
 */
static uint8_t nested_recover_core(mf1_nested_core_t *pnc, uint64_t keyKnown, uint8_t blkKnown, uint8_t typKnown,
                                   uint8_t targetBlock, uint8_t targetType)
{
    struct Crypto1State mpcs = {0, 0};
    struct Crypto1State *pcs = &mpcs;
    uint8_t status;
...
/**
 * @brief    :NESTED is implemented by default to collect random numbers of the sets_nr group. This function is only
 * responsible for collecting, not responsible for conversion and analysis as KS
 * @param    :keyKnown    : The U64 value of the known secret key of the card
 * @param    :blkKnown    :The owner of the known secret key of the card
 * @param    :typKnown    : Types of the known secret key of the card, 0x60 (A secret) or 0x61 (B secret)
 * @param    :targetBlock : The target sector that requires a Nested attack
 ...
 *
 */
uint8_t nested_recover_key(uint64_t keyKnown, uint8_t blkKnown, uint8_t typKnown, uint8_t targetBlock,
                           uint8_t targetType, mf1_nested_core_t ncs[SETS_NR])
{
    uint8_t m, res;
    ...
    for (m = 0; m < SETS_NR; m++) {
        res = nested_recover_core(&(ncs[m]), keyKnown, blkKnown, typKnown, targetBlock, targetType);
        if (res != STATUS_HF_TAG_OK) {
            return res;
        }
    }
 ...
 *               This function is only responsible for collection and is not responsible for converting and parsing to
 * KS.
 ...
 * @param    :nestedAgain : StaticNested enhanced vulnerability, which can obtain two sets of encrypted random numbers
 * based on nested verification of known keys
 * @retval   : Successfully collected and returned to STATUS_HF_TAG_OK, otherwise an error code will be returned.
 *
 */
uint8_t static_nested_recover_core(uint8_t *p_nt1, uint8_t *p_nt2, uint64_t keyKnown, uint8_t blkKnown,
                                   uint8_t typKnown, uint8_t targetBlock, uint8_t targetType, uint8_t nestedAgain)
{
    struct Crypto1State mpcs = {0, 0};
    struct Crypto1State *pcs = &mpcs;
    uint8_t status, len;
...
/**
 * @brief    : StaticNested encapsulates and calls the functions implemented by the core to collect 2 sets of random
 * numbers. This function is only responsible for collection and is not responsible for converting and parsing to KS.
 * @param    :keyKnown    : U64 value of the known key of the card
 * @param    :blkKnown    : The sector to which the card's known secret key belongs
 * @param    :typKnown    : The known key type of the card, 0x60 (A key) or 0x61 (B key)
 * @param    :targetBlock : Target sectors that require nested attacks
 ...
 *
 */
uint8_t static_nested_recover_key(uint64_t keyKnown, uint8_t blkKnown, uint8_t typKnown, uint8_t targetBlock,
                                  uint8_t targetType, mf1_static_nested_core_t *sncs)
{
    uint8_t res;
    res = pcd_14a_reader_scan_auto(p_tag_info);
    if (res != STATUS_HF_TAG_OK) {
    ...
    res = static_nested_recover_core(sncs->core[0].nt1, sncs->core[0].nt2, keyKnown, blkKnown, typKnown, targetBlock,
                                     targetType, false);
    if (res != STATUS_HF_TAG_OK) {
        return res;
    }
    res = static_nested_recover_core(sncs->core[1].nt1, sncs->core[1].nt2, keyKnown, blkKnown, typKnown, targetBlock,
                                     targetType, true);
    if (res != STATUS_HF_TAG_OK) {
        return res;
    }
...
uint16_t mf1_toolbox_check_keys_of_sectors(mf1_toolbox_check_keys_of_sectors_in_t *in,
 ...
 * @retval : STATUS_HF_TAG_OK is returned if the acquisition is successful, and non-HF_TAG_OK is returned if the
 * acquisition is unsuccessful Value
 *
 */
uint8_t mf1_hardnested_nonces_acquire(bool slow, uint8_t blkKnown, uint8_t typKnown, uint64_t keyKnown,
                                      uint8_t targetBlk, uint8_t targetTyp, uint8_t *nonces, uint16_t noncesMax,
                                      uint8_t *num_nonces)
{
    struct Crypto1State mpcs = {0, 0};
    struct Crypto1State *pcs = &mpcs;
...
//-----------------------------------------------------------------------------
uint8_t mf1_static_encrypted_nonces_acquire(uint64_t keyKnown, uint8_t sector_count, uint8_t starting_sector,
                                            uint8_t sector_data[40][sizeof(mf1_static_nonce_sector_t)],
                                            uint8_t *sectors_acquired, uint32_t *cardUid)
{
    struct Crypto1State mpcs = {0, 0};
    struct Crypto1State *pcs = &mpcs;
    ...
    *sectors_acquired = 0;
    ...
    for (uint16_t sec = starting_sector; sec < sector_count && sec < 40; sec++) {
        ...
        for (uint8_t keyType = 0; keyType < 2; keyType++) {
            ...
            uint8_t cmd_status;
            uint8_t res = send_cmd(pcs, AUTH_NESTED, 0x60 + keyType + 4, blockNo, &cmd_status, receivedAnswer, par_enc,
                                   sizeof(receivedAnswer) * 8);
            if (res != 32) {
                return STATUS_MF_ERR_AUTH;
            }
            ...
            res = send_cmd(pcs, AUTH_NESTED, 0x60 + keyType, blockNo, &cmd_status, receivedAnswer, par_enc,
                           sizeof(receivedAnswer) * 8);
            if (res != 32) {
                return STATUS_MF_ERR_AUTH;
            }
...
uint16_t mf1_toolbox_check_keys_on_block(mf1_toolbox_check_keys_on_block_in_t *in,
