...
bool fds_read_sync(uint16_t id, uint16_t key, uint16_t *length, uint8_t *buffer)
{
    ...
    if (fds_find_record(id, key, &record_desc)) {
        err_code
            = fds_record_open(&record_desc, &flash_record);  // Open the record so that it is marked as the open state
        ...
        if (flash_record.p_header->length_words * 4 <= *length) {  // Read the data in Flash here to the given RAM
            ...
            *length = flash_record.p_header->length_words * 4;
            ...
            NRF_LOG_INFO("FDS buffer too small, can't run memcpy, fds size = %d, buffer size = %d",
                         flash_record.p_header->length_words * 4, *length);
        }
    ...
    if (err_code == NRF_SUCCESS) {
        while (!fds_operation_info.success) {
            __NOP();
             ...
             == FDS_ERR_NO_SPACE_IN_FLASH) {  // Make sure there is space to operate, otherwise GC will be required
        ...
        if (err_code == NRF_SUCCESS) {
            ...
            // After gc once, I found that there is still no space, so it may be that the developer did not consider the
            // space distribution and caused overflow
            NRF_LOG_ERROR("FDS no space to write.");
            ret = false;
        }
        else {
            // If it is not an error with insufficient space, then we need Catch the error, and we solve it during
            // development
            APP_ERROR_CHECK(err_code);
        }
    }
...
static void fds_evt_handler(fds_evt_t const *p_evt)
{
    ...
    if (fds_operation_info.ignore_pm
        && (is_peer_manager_record(p_evt->write.record_key) || is_peer_manager_record(p_evt->write.file_id)
            || is_peer_manager_record(p_evt->del.record_key) || is_peer_manager_record(p_evt->del.file_id))) {
        return;
    }
    ...
    switch (p_evt->id) {
        ...
        case FDS_EVT_UPDATE: {
            if (p_evt->result == NRF_SUCCESS) {
                NRF_LOG_INFO("Record change: FileID 0x%04x, RecordKey 0x%04x", p_evt->write.file_id,
                             p_evt->write.record_key);
                if (p_evt->write.file_id == fds_operation_info.id
                    && p_evt->write.record_key == fds_operation_info.key) {
                    ...
                    NRF_LOG_INFO("Record change mismatch");
            }
            else {
        ...
        case FDS_EVT_DEL_RECORD: {
            if (p_evt->result == NRF_SUCCESS) {
                NRF_LOG_INFO("Record remove: FileID: 0x%04x, RecordKey: 0x%04x, RecordID: %08x", p_evt->del.file_id,
                             p_evt->del.record_key, p_evt->del.record_id);
                if (p_evt->del.record_id == fds_operation_info.record_id) {
                    ...
                    NRF_LOG_INFO("Record delete mismatch");
            }
            else {
