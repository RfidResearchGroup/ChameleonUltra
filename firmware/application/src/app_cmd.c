...
static data_frame_tx_t *cmd_processor_mf0_ntag_write_emu_page_data(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data)
{
    ...
    if (nr_pages <= 0)
        return data_frame_make(cmd, STATUS_INVALID_SLOT_TYPE, 0, data);
    ...
    if (pages_count == 0)
        ...
        (page_index >= ((int)nr_pages)) || (pages_count > (((int)nr_pages) - page_index)) || (((int)length - 2) < byte_length))
    {
        byte = nr_pages;
        return data_frame_make(cmd, STATUS_PAR_ERR, 1, &byte);
    }
...
{
    if (length != NFC_TAG_MF0_NTAG_SIG_SIZE)
        return data_frame_make(cmd, STATUS_PAR_ERR, 0, NULL);
...
static cmd_data_map_t m_data_cmd_map[] = {
    ...
    {DATA_CMD_FUZZER_MODE, NULL, cmd_processor_fuzzer_mode, NULL},
    ...
    // Register value block extraction command
    {DATA_CMD_MF1_EXTRACT_VALUE_BLOCKS, NULL, cmd_processor_mf1_extract_value_blocks, NULL},
