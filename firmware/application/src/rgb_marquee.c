...
nrf_drv_pwm_config_t pwm_config = {  // PWM configuration structure
    .irq_priority = APP_IRQ_PRIORITY_LOWEST, .base_clock = NRF_PWM_CLK_1MHz,
    .count_mode = NRF_PWM_MODE_UP,           .top_value = PWM_MAX,
...
void rgb_marquee_init(void) { timer = bsp_obtain_timer(0); }
...
// 4 Lights Dragon Tail horizontal movement cycle (not returning), including the disappearance of the tail and the head
// of the head slowly
// dir 0-from 1 card slot to 8 card slot, 1-from 8 card slot to 1 card slot (Direction, the end point is determined by
// the END parameter) end To scan the number of lamps, decide the final animation area with the direction
        ...
        if (setled < 3) {  // During the positive period, only the first few LEDs can be on during 0, 1, 2
            ...
            for (i = 0; i < leds2turnon; i++) {
                pwm_config.output_pins[3 - i] = led_pins_arr[setled - i];
                 ...
                 <= 7) {  // During the positive period, it can light up 4 LEDs when it is greater than 4 less than 4
...
void ledblink3(uint8_t led_down, uint8_t color_led_down, uint8_t led_up, uint8_t color_led_up)
{
    ...
    if (led_down >= 0 && led_down <= 7) {
        ...
        while (light_level >= 0) {
            ...
            while (callback_waiting == 0)
                ;  // Waiting for the output of the PWM module to complete
            bsp_delay_us(1234);
            callback_waiting = 0;
            light_level--;
        }
...
// 4 Light Tail horizontal movement cycle (not returning), does not include the disappearance of the tail, but includes
// the head of the head (for the type of playback type animation)
// dir 0-from 1 card slot to 8 card slot, 1-from 8 card slot to 1 card slot (Direction, the end point is determined by
// the END parameter) end To scan the number of lamps, decide the final animation area with the direction start_light
// stop_light 0-99 Indicate gradient brightness
 ...
 */
bool is_rgb_marquee_enable(void) { return g_usb_led_marquee_enable; }
